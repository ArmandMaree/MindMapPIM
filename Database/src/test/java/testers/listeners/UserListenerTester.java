...
public class UserListenerTester extends AbstractTester {
	...
	public void testReceiveCheckIfRegistered() throws InterruptedException {
		User user1 = new User("Acuben", "Cos");
		user1.addPimId("gmail", "acubencos@gmail.com");
		...
		User user2 = new User("Acuben", "Cos");
		user2.addPimId("gmail", "acubencos@gmail.com");
	...
	@Test
	public void testReceiveUserUpdateNoUser() throws InterruptedException {
	...
	@Test
	public void testReceiveUserUpdateNoUpdate() throws InterruptedException {
		UserIdentified user = new UserIdentified(UUID.randomUUID().toString(), false, "Acuben", "Cos");
		...
		Assert.assertEquals("Failure - userInRepo's pimIds changed and shouldn't have.", user.getPimIds().size(), userInRepo.getPimIds().size());
		Assert.assertEquals("Failure - userInRepo's gmailId changed and shouldn't have.", user.getPimId("gmail"), userInRepo.getPimId("gmail"));
	...
	public void testReceiveUserUpdate() throws InterruptedException {
		...
		UserIdentified updatesUser = new UserIdentified(UUID.randomUUID().toString(), user.getUserId());
		updatesUser.addPimId("gmail", "test@example.com");
		rabbitTemplate.convertAndSend(TestContext.userUpdateRequestQueueName, updatesUser);
		...
		User userInRepo = userRepository.findByUserId(user.getUserId());
		Assert.assertEquals("Failure - userInRepo's gmailId did not change but should have.", updatesUser.getPimId("gmail"), userInRepo.getPimId("gmail"));
